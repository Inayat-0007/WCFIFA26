...
export const forgotPassword = asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const clientUrl = process.env.CLIENT_URL || 'http://localhost:3000';
    const resetLink = `${clientUrl}/login?resetToken=${token}`;

    console.log(`[PASSWORD RESET] Generated reset token for ${email}: ${resetLink}`);
...
export const resetPassword = asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (decoded.purpose !== 'reset-password' || !decoded.email) {
      res.status(400).json({ success: false, message: 'Invalid reset token purpose' });
      return;
    }
